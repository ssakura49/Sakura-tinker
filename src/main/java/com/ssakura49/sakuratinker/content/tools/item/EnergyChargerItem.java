...
//        IEnergyStorage energy = stack.getCapability(ForgeCapabilities.ENERGY);
//        if (!energy.isPresent()) return;
//        switch (tool.getPersistentData().getInt(CHARGE_MODE)) {
//            case MODE_HANDS -> chargeHands(player, (IEnergyStorage) energy);
//            case MODE_INVENTORY -> chargeInventory(player, (IEnergyStorage) energy);
//            case MODE_CURIOS -> chargeCurios(player, (IEnergyStorage) energy);
//            case MODE_ALL -> chargeAll(player, (IEnergyStorage) energy);
//        }
    }
    ...
    private void transferEnergy(ItemStack target, IEnergyStorage source) {
        ...
        target.getCapability(ForgeCapabilities.ENERGY).ifPresent(targetEnergy -> {
            int maxTransfer = Math.min(source.extractEnergy(Integer.MAX_VALUE, true), 5000);
